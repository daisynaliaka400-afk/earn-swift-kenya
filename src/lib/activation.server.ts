...
export async function notifyActivation(db: SupabaseClient<any>, r: Record<string, unknown> | null) {
  ...
  await sendSms(db, {
    ...
    message: `${r["name"]}, payment of KSh ${r["amount"]} received! You're ACTIVE. Referral link: ${SITE_URL}/register?ref=${r["code"]}`,
  });
  if (r["referrer_id"]) {
    await sendSms(db, {
      phone: String(r["referrer_phone"]), userId: String(r["referrer_id"]), trigger: "commission", dedupeKey: `commission:${r["user_id"]}`,
      message: `${r["referrer_name"]}, you earned KSh ${r["commission"]} from ${r["name"]}! Keep sharing -> ${SITE_URL}/register?ref=${r["referrer_code"]}`,
    });
  }
}
