...
export const sendWelcomeSms = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    await sendSms(db, { phone: u.phone, userId: context.userId, trigger: "welcome", dedupeKey: `welcome:${context.userId}`,
      message: `🎉 Welcome ${u.name}! Your KSh 45 FREE bonus is ready. Login → smartearn.co.ke/login (use your phone number).` });
    if (u.referred_by) {
      ...
      if (r) await sendSms(db, { phone: r.phone, userId: r.id, trigger: "referral_registered", dedupeKey: `ref_reg:${context.userId}`, marketing: true,
        message: `👥 ${r.name}, someone just registered using your link! They'll earn you KSh 80-250 once they activate. Keep sharing → smartearn.co.ke/ref/${r.referral_code}` });
    }
    return { ok: true };
  });
